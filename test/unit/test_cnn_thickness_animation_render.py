...
def test_create_rollout_comparison_animation_uses_truth_color_limits(monkeypatch, tmp_path: Path):
    ...
    def fake_render_frame(*, dataset, frame_index, fields, x, y, time_day, color_limits):
        captured["vmin"] = color_limits["layer_thickness"][0]
        captured["vmax"] = color_limits["layer_thickness"][1]
...
def test_create_rollout_comparison_animation_supports_velocity_panels(tmp_path: Path):
    rollout_path = tmp_path / "rollout.nc"
    dataset = xr.Dataset(
        data_vars={
            "truth_layer_thickness": (("time_days", "y", "x"), np.ones((1, 2, 2), dtype=np.float32)),
            "rollout_layer_thickness": (("time_days", "y", "x"), np.zeros((1, 2, 2), dtype=np.float32)),
            "truth_zonal_velocity": (("time_days", "y", "x"), np.ones((1, 2, 2), dtype=np.float32)),
            "rollout_zonal_velocity": (("time_days", "y", "x"), np.zeros((1, 2, 2), dtype=np.float32)),
            "truth_meridional_velocity": (("time_days", "y", "x"), np.ones((1, 2, 2), dtype=np.float32)),
            "rollout_meridional_velocity": (("time_days", "y", "x"), np.zeros((1, 2, 2), dtype=np.float32)),
        },
        coords={"time_days": [1.0], "y": [0.0, 1.0], "x": [0.0, 1.0]},
    )
    dataset.to_netcdf(rollout_path)

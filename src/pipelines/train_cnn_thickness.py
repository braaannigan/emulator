...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train and evaluate a tiny CNN thickness emulator.")
    parser.add_argument(
        "--config",
        default="config/emulator/cnn_thickness.yaml",
...
def main() -> int:
    args = parse_args()
    config = load_cnn_thickness_config(Path(args.config))

...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train and evaluate a simplified U-Net thickness emulator.")
    parser.add_argument(
        "--config",
        default="config/emulator/unet_thickness.yaml",
        help="Path to the emulator YAML configuration.",
    )
    parser.add_argument(
        "--source-experiment-id",
        help="Override the source experiment id.",
    )
    parser.add_argument(
        "--experiment-id",
...
def main() -> int:
    args = parse_args()
    config = load_unet_thickness_config(Path(args.config))

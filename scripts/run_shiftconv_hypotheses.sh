...
MANIFEST="${1:-shiftconv_hypothesis_manifest.csv}"
RUN_PREFIX="${2:-$(date -u +%Y%m%dT%H%M%S)-dgsw2l-shiftconv}"
RETURN_BRANCH="${3:-2026-04-25-dgsw2l-shiftconv-setup}"
...
while IFS=, read -r id area branch config_path hypothesis; do
  ...
  PYTHONPATH="${PWD}" .venv/bin/python src/pipelines/train_unet_thickness.py \
    --config "${config_path}" \
    --experiment-id "${experiment_id}"
done < "${MANIFEST}"

git switch "${RETURN_BRANCH}"
echo "Completed shifted-stencil branch sequence."
